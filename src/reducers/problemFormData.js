...
export default (state = initialState, action) => {
  switch (action.type) {
    case "UPDATED_SERIES_DATA":
      const totResistance = action.problemFormData.loops[0].resistors.reduce(
        (tot, resistor) => {
          return (tot += parseInt(resistor.resistance, 10));
        },
        0
      );

      const totCurrent = (
        action.problemFormData.tot_voltage / totResistance
      ).toFixed(2);
      ...
      let newResistorArray = resistorArray.map((resistor, index) => {
        return Object.assign({}, resistor, {
          num: index + 1,
          current: totCurrent,
          voltage: (totCurrent * resistor.resistance).toFixed(2),
        });
      });
    ...
    case "UPDATED_PARALLEL_DATA":
      const inverseResistance = action.problemFormData.loops.reduce(
        (tot, loop) => {
          return (tot += 1 / parseInt(loop.resistors[0].resistance, 10));
        },
        0
      );
      ...
      let newParallelLoopArray = action.problemFormData.loops.map((loop) => {
        return Object.assign({}, loop, {
          resistors: loop.resistors.map((r, i) => {
            return Object.assign({}, r, {
              num: i + 1,
              voltage: action.problemFormData.tot_voltage,
              current: (
                action.problemFormData.tot_voltage /
                loop.resistors[i].resistance
              ).toFixed(2),
            });
          }),
        });
      ...
      const totParallelCurrent = newParallelLoopArray.reduce((tot, loop) => {
        return (tot += parseFloat(loop.resistors[0].current));
      }, 0);

      return Object.assign({}, action.problemFormData, {
        tot_current: totParallelCurrent.toFixed(2),
        tot_resistance: totParallelResistance.toFixed(2),
        loops: newParallelLoopArray,
      });
    ...
    case "UPDATED_COMBO_DATA":
      ...
      const comboResistance =
        1 /
        action.problemFormData.loops.reduce((tot, loop) => {
          return (tot +=
            1 /
            loop.resistors.reduce((total, resistor) => {
              return (total += parseInt(resistor.resistance, 10));
            }, 0));
        }, 0);
      ...
      for (var k = 0; k < comboLoopArray.length; k++) {
        comboLoopArray[k].l_resistance = comboLoopArray[k].resistors.reduce(
          (total, resistor) => {
            return (total += parseInt(resistor.resistance, 10));
          },
          0
        ...
        comboLoopArray[k].l_current =
          comboLoopArray[k].l_voltage / comboLoopArray[k].l_resistance;
        for (var r = 0; r < comboLoopArray[k].resistors.length; r++) {
          comboLoopArray[k].resistors[r].num = rCount;
          comboLoopArray[k].resistors[r].current = comboLoopArray[
            k
          ].l_current.toFixed(2);
          comboLoopArray[k].resistors[r].voltage = (
            comboLoopArray[k].l_current *
            comboLoopArray[k].resistors[r].resistance
          ).toFixed(2);
          rCount += 1;
        }
      }
